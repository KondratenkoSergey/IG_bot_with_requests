...
class InstaBot:
    """
    Instagram bot v 1.0
    like_per_day=1000 -Сколько лайков в день

    media_max_like=0 -  "Не лайкать фото/видео,если лайков более чем " " 



    tag_list = ['cat', 'car', 'dog'] - Тег лист для лайков



    log_mod = 0 - Log mod log_mod = 2 no log.
    ...
    def __init__(self, login, password,
                ...
                tag_list=['awesome', 'amazing', 'красота','Закат','супер','кот'],
                max_like_for_one_tag = 300,
                unfollow_break_min=40,
                unfollow_break_max=90,
                log_mod = 0):
        ...
        # Если у вас есть прокси,то пишите его тут,откмментив
        ...
        log_string = 'SMInsta v1.0.1 started at %s:\n' %\
    ...
    def login(self):
        ...
        if login.status_code == 200:
            ...
            self.write_log('Login error! Нет подключения к интернету!')
    ...
    def get_media_id_by_tag (self, tag):
        """  Поиск по ваему хештегу """

        if (self.login_status):
            log_string = "Ищем медиа по тегу: %s" % (tag)
            self.write_log(log_string)
        ...
        if (self.login_status):
            ...
            try:
                ...
                if comment.status_code == 200:
                    ...
                    log_string = 'Написали: "%s". #%i.' % (comment_text, self.comments_counter)
                    self.write_log(log_string)
                return comment
            except:
                self.write_log("Ошибка комментирования!")
        return False
        ...
        if (self.login_status):
            ...
            try:
                ...
                if follow.status_code == 200:
                    ...
                    log_string = "Подписка: %s #%i." % (user_id, self.follow_counter)
                    self.write_log(log_string)
                return follow
            except:
                self.write_log("Ошибка подписки!")
        return False
        ...
        if (self.login_status):
            ...
            try:
                ...
                if unfollow.status_code == 200:
                    ...
                    log_string = "Отписка: %s #%i." % (user_id, self.unfollow_counter)
                    self.write_log(log_string)
                return unfollow
            except:
                self.write_log("Ошибка отписки!")
        return False

    def unfollow_on_cleanup(self, user_id):
        """ Unfollow on cleanup by @Saintmonster """
        if (self.login_status):
            url_unfollow = self.url_unfollow % (user_id)
            try:
                unfollow = self.s.post(url_unfollow)
                if unfollow.status_code == 200:
                    ...
                    log_string = "Подождите 5 минут, иначе забанят!"
                    self.write_log(log_string)
                    time.sleep(300)
                    unfollow = self.s.post(url_unfollow)
                    if unfollow.status_code == 200:
                        self.unfollow_counter += 1
                        log_string = "Отпска: %s #%i of %i." % (user_id, self.unfollow_counter, self.follow_counter)
                        self.write_log(log_string)
                    else:
                ...
                log_string = "Ошибка отписки. Чекните интернет соеддиение"
                self.write_log(log_string)
        return False
    ...
    def write_log(self, log_text):
        ...
        if self.log_mod == 0:
            ...
            try:
                ...
                print("Ваш текст имеет проблемы кодировки!")
